...
export default async function CampaignLorePage({ searchParams }: { searchParams: Promise<{ campaign?: string; category?: string; entity?: string }> }) {
  ...
  if (params.entity) {
    ...
    if (entityData?.entity?.campaign_id === campaignId) return <EntityView data={entityData} categories={categories} />;
  }
  ...
  return (
    <div className="lore-browser">
      ...
      <section className="data-panel lore-entities" id="all-lore">
        ...
        {entities.length ? <div className="entity-grid">{entities.map((entity) => <Link className="entity-card" href={`/data/campaign-lore?campaign=${campaignId}&entity=${entity.id}`} key={entity.id}><span className="campaign-glyph">E</span><h2>{entity.name}</h2><p>{categories.find((category) => category.id === entity.category_id)?.name ?? "Uncategorized"}</p></Link>)}</div> : <div className="empty-state"><h2>No lore entries yet</h2><p>Create an entity or choose another category.</p></div>}
